...
$( document ).ready(function() {
    ...
    <div class="listmenumain">
        <a href="hoso"><i class="fa fa-user"></i> Hồ sơ</a>
        <a href="giaidau"><i class="fa fa-clock"></i> Giải đấu</a>
        <a href="lichtapvdv"><i class="fa fa-calendar"></i> Lịch ngày</a>
...
async function checkroleAthlete() {
    if(token == null){
        window.location.href = '../login'
        return;
    }
    var url = 'http://localhost:8080/api/user/athlete/check-role-athlete';
    const response = await fetch(url, {
        method: 'GET',
        headers: new Headers({
            'Authorization': 'Bearer ' + token
